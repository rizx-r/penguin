...
type (
	friendRequestsModel interface {
		...
		FindByReqUidAndUserId(ctx context.Context, uid, userId string) (*FriendRequests, error)
...
func (m *defaultFriendRequestsModel) FindByReqUidAndUserId(ctx context.Context, uid, userId string) (*FriendRequests, error) {
	query := fmt.Sprintf("select %s from %s where `req_uid` = ? and `user_id` = ? limit 1", friendRequestsRows, m.table)
	var resp FriendRequests
	err := m.QueryRowNoCacheCtx(ctx, &resp, query, uid, userId)
	switch err {
	case nil:
		return &resp, nil
	case sqlc.ErrNotFound:
