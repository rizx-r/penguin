...
type (
	friendsModel interface {
		...
		InsertBatch(ctx context.Context, session sqlx.Session, data ...*Friends) (sql.Result, error)
...
func (m *defaultFriendsModel) InsertBatch(ctx context.Context, session sqlx.Session, data ...*Friends) (sql.Result, error) {
	var (
		sql strings.Builder
		args []any
	)
	...
	// insert into `fun-chat.friends` (user_id, friend_id, remark, add_source) values
	...
	sqlStr := sql.String()
	fmt.Println(sqlStr)
	fmt.Println("执行SQL:", formatSQL(sqlStr, args))
	return session.ExecCtx(ctx, sqlStr, args...)
